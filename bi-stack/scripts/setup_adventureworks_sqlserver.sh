...
BACKUP_DIR="$ROOT/bi-stack/adventureworks/backups"
BAK="$BACKUP_DIR/AdventureWorksDW2022.bak"
URL="${ADVENTUREWORKS_BAK_URL:-https://github.com/Microsoft/sql-server-samples/releases/download/adventureworks/AdventureWorksDW2022.bak}"
SA_PASSWORD="${ADVENTUREWORKS_SA_PASSWORD:-MCAD_AwDWDemo!2026}"
SERVICE="adventureworks-sqlserver"
DB="${ADVENTUREWORKS_SQLSERVER_DATABASE:-AdventureWorksDW2022}"

mkdir -p "$BACKUP_DIR"
echo "=== MCAD V9.5.0 AdventureWorksDW SQL Server Docker setup ==="
echo "repo_root=$ROOT"
echo "backup=$BAK"

if [[ ! -s "$BAK" ]]; then
  echo "Downloading AdventureWorksDW2022.bak from Microsoft sample release..."
  curl -L --fail --retry 5 --retry-delay 2 -o "$BAK" "$URL"
else
  echo "Backup already exists; skipping download."
fi
[[ -s "$BAK" ]] || { echo "ERROR: missing/empty backup: $BAK" >&2; exit 2; }
...
for i in $(seq 1 90); do
  if _sqlcmd_exec "SELECT 1 AS ready" >/dev/null 2>&1; then echo "SQL Server is ready."; break; fi
  [[ "$i" != "90" ]] || { echo "ERROR: SQL Server not ready." >&2; docker logs "$CID" --tail=120 >&2 || true; exit 4; }
  sleep 2
done

echo "Restoring AdventureWorksDW2022 from backup..."
_sqlcmd_file "$ROOT/bi-stack/adventureworks/sql/restore_adventureworksdw2022.sql"

echo "Verifying restored database..."
_sqlcmd_exec "SELECT DB_NAME() AS current_db; SELECT COUNT_BIG(*) AS FactInternetSalesRows FROM [$DB].dbo.FactInternetSales;"
echo "AdventureWorksDW SQL Server Docker setup completed."
