...
TMP_API="/tmp/mcad_r3c3_api_openapi.$$"
TMP_PROXY="/tmp/mcad_r3c3_proxy_openapi.$$"
TMP_HEALTH="/tmp/mcad_r3c3_api_health.$$"
...
fail() {
  echo
  echo "R3_C3_ONE_SHOT_PREMEASUREMENT_GATE=FAIL reason=$1"
...
echo "=== R3-C3 one-shot premeasurement gate (READ-ONLY) ==="
...
[ -n "${R3_AW_SA_PASSWORD:-}" ] || fail "R3_AW_SA_PASSWORD_not_set"
echo "r3_aw_sa_password_present=true"
...
echo
echo "=== 2. Isolated clone exact identity / liveness / mounts ==="
...
echo
echo "=== 3. Read-only HTTP health + route-contract gate ==="
...
echo
echo "=== 4. Output-space and no-redo preconditions ==="
...
echo "R3_C3_ONE_SHOT_PREMEASUREMENT_GATE=PASS"
