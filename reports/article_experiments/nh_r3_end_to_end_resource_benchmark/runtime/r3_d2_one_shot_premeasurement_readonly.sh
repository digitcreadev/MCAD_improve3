...
TMP_API="/tmp/mcad_r3d2_api_openapi.$$"
TMP_PROXY="/tmp/mcad_r3d2_proxy_openapi.$$"
TMP_HEALTH="/tmp/mcad_r3d2_api_health.$$"
...
fail() {
  echo
  echo "R3_D2_ONE_SHOT_PREMEASUREMENT_GATE=FAIL reason=$1"
...
echo "=== R3-D2 one-shot premeasurement gate (READ-ONLY) ==="
...
echo "PRIMARY_300_AUTHORIZED=true"
...
echo
echo "=== 4. Credential / output-space / anti-redo checks ==="
...
existing_attempts="$(find /workspaces -maxdepth 1 \( -type d -o -type f \) \
  \( -name 'MCAD_R3_D3_CONFIRMATORY_PRIMARY_ATTEMPT_*' -o -name 'MCAD_R3_D3_CONFIRMATORY_PRIMARY_RESULTS_*' \) \
  -print | LC_ALL=C sort)"
if [ -n "$existing_attempts" ]; then
  echo "preexisting_D3_paths:"
  printf '%s\n' "$existing_attempts"
  fail "preexisting_D3_attempt_requires_assistant_audit"
fi
...
echo "preexisting_D3_attempts=NONE"
echo "fallback_120_activated=false"
...
echo "R3_D2_ONE_SHOT_PREMEASUREMENT_GATE=PASS"
